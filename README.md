...
└──────┬────────────────┬──────────────────┬───────────┘
  ...
  └──────────┘   └─────────────┘    └─────┬──────┘
                  ...
                  └──┬─────┬───┘  └──┬─────┬────┘  └────────────┘
                     │     │         │     │
               ┌─────▼┐ ┌─▼────┐ ┌──▼──┐ ┌▼──────────┐
               │Yahoo │ │ News │ │Chro-│ │  Market   │
               │ Fin. │ │ Svc  │ │maDB │ │ Enrichment│
               └──────┘ └──────┘ └─────┘ └───────────┘
```
...
嵌入模型热加载： 冷启动后的第一次查询需要 ~3s 加载模型。可以预加载到共享内存，或独立部署嵌入微服务，实现 <100ms 冷启动。
智能缓存策略： 当前使用固定 TTL 的进程内缓存。频率感知缓存（LFU）配合差异化 TTL（蓝筹股 5min、小盘股 1min）可以在不提供过时数据的前提下提高命中率。
...
查询扩展： 当向量检索返回低相关度结果时，自动生成 2-3 个查询变体（如"市盈率"→ "P/E ratio"、"price-to-earnings"）并合并结果。用 LLM 改写成本很低。

架构扩展

多 Worker 部署： 将进程内 TTLCache 替换为 Redis 统一缓存行情和新闻数据。当前会话缓存已使用 Redis，模式可直接复用。
WebSocket 升级： SSE 是单向的。WebSocket 可实现服务端推送，支持价格预警、组合通知和实时协作会话。
流式 Supervisor： 当前 Supervisor 等所有子任务数据收集完毕后才调用合成器。流式变体可以在第一个子任务完成后就开始合成，降低复合查询的感知延迟。
